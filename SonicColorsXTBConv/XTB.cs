...
namespace SonicColorsXTBConv
{
    public static class XTB
    {
        public static void XTBtoXML(string path)
        {
            ...
            for(int i = 0; i < categoriesCount; i++)
            {
                ...
                for(int j = 0; j < CellsCount; j++)
                {
                    ...
                    int charsCount = binaryReader.ReadInt32();  // Charactesr count, but multiplied by 2
                    binaryReader.ReadInt32();                   // The actual characters count without dots
                    cellData.CellTextData = Unicode.GetString(binaryReader.ReadBytes(charsCount));
	    ...
	    binaryReader.Close();
            ...
            foreach(Category category in categories)
            {
                ...
                foreach(Cell cell in category.CellList)
                {
                    ...
                    writer.WriteString(cell.CellTextData.Replace("\n", "$n"));
                    writer.WriteEndElement();
                }
                writer.WriteEndElement();
            }
                        ...
                        foreach(XmlElement categoryNode in node.ChildNodes)
                        {
                            ...
                            foreach(XmlElement cellNode in categoryNode.ChildNodes)
                            {
                                if(cellNode.Name == "Cell")
                                {
                                    ...
                                    // Cell's text data
                                    cell.CellTextData = cellNode.InnerText;
        ...
        public struct Cell
        {
            ...
            public string CellTextData {get;set;}
        }
